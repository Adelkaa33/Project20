...
public class UniversityExample {
    ...
    public static void main(String[] args) {
        ...
        List<Student> graduatedStudents = students.stream()
                // фильтр по сдаче экзамена (фамилия + имя содержится в examPassedNames)
                .filter(student -> examPassedNames.contains(student.surname + " " + student.name))
                // в map устанавливаем название группы по entranceYear
                .map(student -> {
                    student.groupName = groupNames.get(student.entranceYear);
                    return student;
                })
                // добавляем email студента в список выпускников
